...
def VBx(x0, Phi,
        sid2gidlist, gid2sids, gidlist2sid, pov,
        loopProb=0.9, Fa=1.0, Fb=1.0, pi=10, gamma=None, maxIters=10,
        epsilon=1e-4, alphaQInit=1.0, ref=None, plot=True,
    ...
    alphaQInit  - Dirichlet concentraion parameter for initializing gamma ディリクレ分布のハイパラ
    ...
    n_channels = len(x0)
    X = np.hstack(x0)
    D = x0[0].shape[-1]  # feature (e.g. x-vector) dimensionality
    Phi = np.hstack([Phi] * n_channels)
    ...
    def state_stats2gaussian_stats(Ls, gid2sids, sid2gidlist, nCh=2):
        # Accumurate Ls to each Lg： Ls [D*nCh]を状態内の各ガウス分布のLg [L]に積み立てる
        Lg = [0] * len(gid2sids)
        D = int(Ls.shape[-1] / nCh)
        for g, sids in gid2sids.items():
            for sid in sids:
                Lg[g] += Ls[sid, :int(D)]
        Lg = np.array(Lg)
        # list to array
        # Lg to L: Lgを連結してD*nCHのベクトルに変換する
        L = np.zeros([len(sid2gidlist), D * nCh])
        for s in range(len(Ls)):  # Loop over sid
            for ii, gid in enumerate(sid2gidlist[s]):
                L[s, D * ii: D * (ii + 1)] = Lg[gid]
        return L  # [sid x D*nCh]
    ...
    for ii in range(maxIters):
        ...
        if ii > 0 or alpha is None or invL is None:
            # gamma: [frame x sid^2]
            # Ls: [sid^2 x 2D]
            Ls = Fa / Fb * gamma.sum(axis=0, keepdims=True).T * Phi  # for each state
            L = state_stats2gaussian_stats(Ls, gid2sids, sid2gidlist)
            invL = 1.0 / (1 + L)
            #invL = 1.0 / (1 + Fa/Fb * gamma.sum(axis=0, keepdims=True).T*Phi)  # (17) for all speakers
            alpha = Fa/Fb * invL * gamma.T.dot(rho)  # (16) for all speakers [sid^2] x 2D
            alpha = state_stats2gaussian_stats(alpha, gid2sids, sid2gidlist)
